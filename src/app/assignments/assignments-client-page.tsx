...
export function AssignmentsPageContent() {
  ...
  const filteredAssignments = useMemo(() => {
    ...
    const analytics = useMemo(() => {
        ...
        const totalPaid = enrichedAssignments.filter(a => a.paymentStatus === 'Ödendi').reduce((sum, a) => sum + a.assignedAmount, 0);
        const totalOutstanding = totalAssignedValue - totalPaid;
        return { totalAssignedValue, totalPaid, totalOutstanding };
    }, [enrichedAssignments]);

  const handleStatusChange = async (assignmentId: string, newStatus: JobAssignment['paymentStatus']) => {
    if (!firestore) return;
    try {
      await updateDoc(doc(firestore, 'job_assignments', assignmentId), { paymentStatus: newStatus });
  ...
  return (
    <div className="flex flex-col gap-4 p-8">
      <div className="flex items-center justify-between space-y-2">
        <div>
          <h2 className="text-2xl font-bold tracking-tight">İş Atamaları</h2>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Anlaşılan Tutar</TableHead>
              ...
              {isLoading ? (
                ...
                filteredAssignments.map((a) => (
                  <TableRow key={a.id}>
                    ...
                    <TableCell className="font-mono font-semibold">{formatCurrency(a.assignedAmount)}</TableCell>
                    ...
                    <TableCell className="text-right">
                       <DropdownMenu>
                            ...
                            <DropdownMenuContent>
                                {paymentStatusOptions.map(status => (
                                    <DropdownMenuItem key={status} onClick={() => handleStatusChange(a.id, status)}>
                                        Durumu "{status}" olarak değiştir
                                    </DropdownMenuItem>
                                ))}
                            </DropdownMenuContent>
                       </DropdownMenu>
